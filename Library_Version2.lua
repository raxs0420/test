...
-- run_vote_skip and other helper functions are used in handle_post_match
...
function TDS:Mode(difficulty)
    -- TDS:Mode will attempt to start matchmaking; this function expects to be run while in lobby
    -- We don't strictly rely on the stale game_state here; allow attempt but return false if not lobby UI
    local lobby_hud = player_gui:FindFirstChild("ReactLobbyHud")
    if not lobby_hud and game_state ~= "LOBBY" then 
        -- attempt to detect lobby UI dynamically
        local pl = game:GetService("Players").LocalPlayer
        if pl and pl:FindFirstChild("PlayerGui") then
            local pg = pl.PlayerGui
            if pg:FindFirstChild("ReactLobbyHud") then
                lobby_hud = pg.ReactLobbyHud
            end
        end
    end
    ...
    local frame = lobby_hud and lobby_hud:FindFirstChild("Frame")
    local match_making = frame and frame:FindFirstChild("matchmaking")
...
-- // handle_post_match: improved restart attempts + fallback automatic activation
local function handle_post_match()
    ...
    -- Start new match with stored difficulty, with retry attempts and fallback activation if restart fails
    if current_difficulty then
        ...
        for i = 1, attempts do
            ...
            if ok then
                -- wait short while to see if lobby transitions to in-game UI
                local detect_wait = 0
                local detect_max = 20
                repeat
                    task.wait(0.5)
                    ...
                    if pg and (pg:FindFirstChild("ReactIngameHud") or pg:FindFirstChild("GameGui")) then
                        started = true
                        break
                    end
                until detect_wait >= detect_max
            else
                warn("TDS:Mode pcall failed: " .. tostring(err))
            end
        ...
        if not started then
            -- fallback: enable auto-activation features so the script stays active even if matchmaking couldn't be started
            warn("Auto-restart failed after attempts; enabling fallback automatic features.")
            _G.AutoChain = true
            _G.AutoDJ = true
            _G.AutoPickups = true
            _G.AutoSkip = true
            ...
            -- start the helper loops immediately
            pcall(function()
                start_auto_chain()
                start_auto_dj_booth()
                start_auto_pickups()
            ...
            -- try to ensure we're in a lobby state by sending back to lobby as a last resort
            pcall(function()
                send_to_lobby()
            end)
        else
...
-- rest of file (unchanged) - voting, map selection, anti-afk, loops and starts

-- // voting & map selection (functions already defined above but declared again for clarity if needed)
-- (The previously defined functions run_vote_skip, match_ready_up, cast_map_vote, lobby_ready_up, select_map_override, cast_modifier_vote, is_map_available are used)
