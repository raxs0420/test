...
local function match_ready_up()
    ...
    -- NEW: Execute pending strategy if exists
...
-- NEW: Check if a specific map is available
...
-- NEW: Veto current maps and wait for new ones
...
-- NEW: Get all currently available maps
...
-- Register a strategy for a specific map
...
-- Find first available map from a list
function TDS:FindAvailableMap(mapList)
    -- First check currently available maps
    local availableMaps = get_available_maps()
    
    for _, mapName in ipairs(mapList) do
        for _, availableMap in ipairs(availableMaps) do
            if availableMap == mapName then
                return mapName
            end
        end
    end
    
    -- If no maps found, try veto once
    print("No preferred maps found, trying veto...")
    veto_and_wait_for_maps()
    
    -- Check again after veto
    availableMaps = get_available_maps()
    for _, mapName in ipairs(mapList) do
        for _, availableMap in ipairs(availableMaps) do
            if availableMap == mapName then
                return mapName
...
-- Override GameInfo to use the new system (optional)
function TDS:GameInfo(mapName, modifiers)
    return original_game_info(mapName, modifiers)
end

-- Helper: Check if a map is available (exposed to user)
function TDS:IsMapAvailable(mapName)
    return is_map_available(mapName)
end

-- Helper: Get all available maps
function TDS:GetAvailableMaps()
    return get_available_maps()
end

-- Helper: Veto current maps
